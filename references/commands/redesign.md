...
2. Load `references/memory/quality-bar.md` — a full-page redesign is judged against the whole checklist, not spot-checked. Also load `references/memory/ui-foundations.md`.
...
7. If a design direction isn't already established for this project, run `references/commands/styles.md` and confirm it with the user before redesigning — a page-level redesign without an agreed direction produces inconsistent output.
8. If the page is Arabic/bilingual, also load `references/memory/typography-arabic.md` and `references/memory/rtl-css-engineering.md` — re-run the per-component RTL checklist across every section, not just the ones that visibly prompted the redesign.
