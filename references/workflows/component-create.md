...
0. **Step 0 One-Line Design Read & Dial Configuration**: Declare the component's core design intent in a single sentence. Configure the 3-Dial System (`designVariance`, `motionIntensity`, `visualDensity`).
...
5. **Run Pre-Emit Self-Critique (1-5 Scoring on 6 Axes)**: Score the proposed component on Philosophy (P), Hierarchy (H), Execution (E), Specificity (S), Restraint (R), and Variety (V). Any score < 3 triggers an immediate revision. Stamp the output header/comment: `/* Pre-emit critique: P5 H4 E5 S4 R5 V5 */`.
